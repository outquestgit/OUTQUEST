...
// GA4 page_view for SPA navigation (showPage doesn't change the URL, so the
// React route tracker can't see it). Deduped by path so the initial page —
// already counted by GA's config call — isn't double-counted.
var _gaLastPath = (typeof window !== 'undefined' && window.location && window.location.pathname) || '/';
function _gaPageview(id){
  if(typeof window.gtag!=='function')return;
  var path=(!id||id==='home')?'/':'/'+id;
  if(path===_gaLastPath)return;
  _gaLastPath=path;
...
// Admin-editable Global Copy (Settings → Global Copy), injected as JSON by the
// <Overlays> server component. Read lazily so it's always current after nav.
function gcopy(){
  try{const el=document.getElementById('global-copy-data');return el?JSON.parse(el.textContent||'{}'):{};}catch(e){return{};}
}

function openQuestModal(icon,title,desc){
  const _gc=gcopy();
  document.getElementById('modal-icon').textContent=icon||'🗺️';
  document.getElementById('modal-title').textContent=title||_gc.questModalHeading||'Start your quest';
  document.getElementById('modal-desc').textContent=desc||_gc.questModalSubtext||'Tell us a bit about yourself and we\'ll send you everything you need.';
...
function openDealPage(dealId){
  ...
  // Unknown slug (e.g. a real DB deal not in this demo map): navigate to the
  // server-rendered deal page instead of doing nothing, so the card is clickable.
  if(!deal){if(dealId&&dealId!=='fallback'){window.location.href='/deals/'+encodeURIComponent(dealId);}return;}
...
function _readQuizData(){
  ...
  return {enabled:true,showOnHomepage:true,showOnQuests:true,resultsDisplay:'top3',progression:'one'};
}
...
// Quests + their Category (activity/type), Budget and Duration term slugs,
// injected as JSON <script id="quiz-quests-data"> for result filtering.
function _readQuizQuests(){
  try{
    var el=document.getElementById('quiz-quests-data');
    if(el){var d=JSON.parse(el.textContent||'[]');if(Array.isArray(d))return d;}
  }catch(e){}
  return [];
}
// Does quest `q` carry the term `slug` for taxonomy `kind`?
function _quizHasTerm(q, kind, slug){
  var list = kind==='category' ? (q.categories||[])
           : kind==='budget'   ? (q.budgets||[])
           : kind==='duration' ? (q.durations||[])
           : [];
...
function showQuizResults(){
  ...
  document.querySelectorAll('#quiz-overlay .quiz-opt.selected').forEach(function(el){
    ...
    if(kind&&slug){
      if(!filters[kind]) filters[kind]=slug;
  ...
  if(container){
    container.innerHTML=picks.map(function(q,i){
      var cls=i===0?'primary':(i===1?'alt':'wild');
      return '<div class="qpath-card '+cls+'" data-quiz-quest="'+_qEsc(q.slug)+'">'+
        '<div class="qpath-icon">'+_qEsc(q.icon||'🌍')+'</div>'+
        '<div class="qpath-body">'+
          '<div class="qpath-badge">'+badges[Math.min(i,2)]+'</div>'+
          '<div class="qpath-title">'+_qEsc(q.title)+'</div>'+
          (q.meta?'<div class="qpath-why">'+_qEsc(q.meta)+'</div>':'')+
        '</div>'+
      '</div>';
    }).join('') || '<div style="text-align:center;padding:24px;color:var(--text2);font-size:14px">No matching quests yet — try “Browse all quests”.</div>';
    container.querySelectorAll('.qpath-card').forEach(function(card){
      var slug=card.getAttribute('data-quiz-quest');
      card.onclick=function(){ closeQuiz(); window.location.href='/quests/'+slug; };
    });
  }

  // Match chips (the filtering answers the visitor chose).
  var tagEl=document.getElementById('quiz-match-tags');
  if(tagEl){
    tagEl.innerHTML=chips.map(function(c){
      return '<span style="display:inline-flex;align-items:center;gap:5px;background:rgba(232,69,26,.08);border:1px solid rgba(232,69,26,.2);color:var(--orange);border-radius:50px;padding:4px 12px;font-size:11px;font-weight:700;">✓ '+_qEsc(c)+'</span>';
    }).join('');
  }
  ...
  // Hide the (all-visible / snap) question list once results are shown.
  var box=document.getElementById('quiz-box'); if(box){ box.classList.add('quiz-show-results'); box.scrollTop=0; }
}
...
function retakeQuiz(){
  ...
  _quiz.multi3=[];
  window._lastQuizPaths=[];
  window._lastQuizQuests=[];
  // Apply the admin "Progression style" (one / all / snap) and clear the
  // results state so the questions show again.
  var box=document.getElementById('quiz-box');
  if(box){ box.setAttribute('data-prog', _readQuizData().progression||'one'); box.classList.remove('quiz-show-results'); }
  var sb=document.getElementById('quiz-save-all-btn');
  if(sb){sb.classList.remove('saved-all');sb.textContent='🔖 Save these to My Quests';}
...
function sqCompareQuizMatches(){
  // Compare the actual matched quests (their own data), not the generic paths.
  var quests=(window._lastQuizQuests||[]).slice(0,3);
  closeQuiz();
  sqOpenQuizCompareModal(quests);
}

// Build the side-by-side markup for the 3 quiz-result quests using each quest's
// own fields (injected via #quiz-quests-data in SiteApp.tsx).
function sqBuildQuizCompareMarkup(quests){
  var list=(quests||[]).slice(0,3);
  var esc=function(s){return String(s==null?'':s).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;');};
  if(!list.length){
    return '<div class="sq-compare-empty"><div><strong>No matches yet.</strong><p>Take the quiz to find quests, then compare your top matches here.</p></div></div>';
  }
  var badges=['Best match','Strong match','Worth exploring'];
  var row=function(label,val){return '<div class="sq-compare-row"><span class="sq-compare-label">'+label+'</span><span class="sq-compare-value">'+esc(val||'—')+'</span></div>';};
  return '<div class="sq-compare-columns" style="--sq-cols:'+list.length+'">'+list.map(function(q,i){
    return '<article class="sq-compare-result" data-slug="'+esc(q.slug)+'">'+
      '<div class="sq-compare-result-head"><div class="sq-compare-score">'+badges[Math.min(i,2)]+'</div><div><div style="font-size:32px;margin-bottom:10px;">'+esc(q.icon||'🌍')+'</div><div class="sq-compare-result-name">'+esc(q.title)+'</div><div class="sq-compare-result-fit">'+esc(q.meta||'')+'</div></div></div>'+
      '<div class="sq-compare-metrics">'+
      row('Cost',q.cost)+row('Time',q.time)+row('Effort',q.effort)+row('Location',q.location)+row('Best time',q.bestTime)+
      '<div class="sq-compare-note"><a href="/quests/'+esc(q.slug)+'" style="color:var(--orange);font-weight:700;text-decoration:none;">View quest →</a></div>'+
      '</div></article>';
  }).join('')+'</div>';
}

function sqOpenQuizCompareModal(quests){
  var modal=sqEnsureCompareModal();
  var out=document.getElementById('sqCompareModalOutput');
  if(out) out.innerHTML=sqBuildQuizCompareMarkup(quests);
...
function sqEnsureCompareModal(){
  ...
  const _ch=_esc(_gc.compareHeading||'Compare your paths');
  const _cs=_esc(_gc.compareSubtext||'See cost, time, income, risk, difficulty, and first step side by side before choosing your next move.');
  modal.innerHTML='<div class="sq-compare-modal-box" role="dialog" aria-modal="true" aria-label="Compare paths"><div class="sq-compare-modal-head"><div><h3>'+_ch+'</h3><p>'+_cs+'</p></div><button class="sq-compare-modal-close" type="button" onclick="sqCloseCompareModal()">×</button></div><div class="sq-compare-modal-body"><div id="sqCompareModalOutput"></div><div class="sq-compare-modal-actions"><button class="sq-modal-add-more" type="button" onclick="sqCloseCompareModal();sqScrollToCompare();">Add or change paths</button><button class="sq-modal-clear" type="button" onclick="sqClearCompare();sqCloseCompareModal();">Clear comparison</button></div></div></div>';
